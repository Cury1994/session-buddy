...
| FR-3.11 | 批准压制二问 | 用户在工具批准 → approve.sh 输出 `hookSpecificOutput.permissionDecision:"allow"` + exit 0，权限引擎跳过终端原生询问，审批点唯一在工具（2026-08-03 01:26 零干扰实测确认本机 2.1.207 尊重该格式，用户目视确认没弹直接执行）；**拦截不托付 JSON** 而用 exit 2（解析失败 fail-open 是安全洞，fail-safe）；批准为一次性语义（同命令重复仍弹卡，永久化见 §5 延后项 D4） | P0 |
