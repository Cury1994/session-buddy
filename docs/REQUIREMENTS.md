...
> 版本 v2.1 | 2026-07-23 | Electron 全量重写
>
...
| FR-1.2 | 余额卡片 | 每 provider 一张信息卡片，展示：剩余余额、货币单位、今日 token 消耗、本月已用金额、千 token 均价 | P0 |
| FR-1.3 | 30天趋势图 | 折线图展示近 30 天每日 token 消耗趋势，含图例、网格线、坐标轴标签 | P1 |
| FR-1.4 | 余额告警 | 当余额低于可配阈值（默认 15%），托盘图标变红，同时发送桌面通知 | P0 |
...
| FR-3.9 | 终端并行 | Hook 脚本同时输出审批提示到终端，用户可在终端直接 Ctrl+C 拒绝，或另开窗口 curl 响应（谁先响应谁赢） | P2 |
...
| FR-5.2 | 通用设置 | 开机自启开关、通知开关 | P1 |
| FR-5.3 | Provider 设置 | 余额告警阈值 | P1 |
| FR-5.4 | 审批设置 | 审批超时时间 | P2 |
...
| FR-6.6 | 开机自启 | 可选，写 `~/.config/autostart/harness-monitor.desktop` | P2 |
...
| ID | 故事 | 验收标准 |
|----|------|---------|
| US-1 | 作为开发者，我打开托盘面板就能看到 DeepSeek 还剩多少钱，不用每次 curl API | 用量 Tab 显示余额卡和 30 天趋势 |
