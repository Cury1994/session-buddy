...
- 2026-08-06 凌晨 heredoc 过度镜像修复（43cb535：splitTopLevel 按块切分 heredoc，消除 `python3 - <<'EOF'` 误弹卡）——详见日志末条
...
### 2026-08-06 01:22:31 ｜ 归档后修订 ｜ heredoc 过度镜像修复 完成（commit 43cb535）
- 背景：用户反馈"工具最近弹了 2 次审批，终端里却没弹"，期望「终端弹的审批才出现在工具中，其他不出现」。只读排查定位：本会话调查命令（Bash，01:08/01:11 两批，approval_history id 213-221）弹卡，其中 curl/python 子命令**早被 `Bash(curl *)` / `Bash(python3 *)` 通配覆盖**，非其所致；复合命令弹卡源于逐段 every-quantifier 遇到未覆盖段（裸 echo/ss/grep/cd）——终端默认模式**确实会弹**，属正确行为。
- **真·过度镜像 = heredoc**：`splitTopLevel` 按换行把 `python3 - <<'EOF'` 正体拆成多段，每段匹配不上前缀规则 → 工具误判 ask 弹卡；而终端把整条当一条命令、命中 `Bash(python3 *)` 实际**静默**。这是乱 mirrorFilter 设计目标（工具面 ≡ 终端面）的 bug，只有改代码能治。
- 实现（开发+测试合并于主对话，S 档轻量）：splitTopLevel 增加 heredoc 状态机——识别 `<<DELIM`…`DELIM`（支持 `<<-` tab 收尾 / 单引号 / 部分引号 `<<E"O"F`）为单个子命令；终止符独占一行处收段（后续裸换行也正确断段）；分隔符读取恒定在空白/`;`/`<>` 处终止。
- 验证全绿：16 项分段用例全 PASS（复合 &&/;/|/|| / 引号内分隔符 / heredoc 单块 / `<<-` tab / 部分引号 / heredoc+ && / 多 heredoc 裸换行断段 / 未闭合不崩 / `<<<` here-string 不误判）；镜像集成——`python3 - <<'EOF'`→**passthrough**（修复点）、普通 `python3 -c`→passthrough、`echo && curl`→ask（正确）、`sudo rm -rf`→ask、**`bash <<EOF`+`sudo rm`→ask（危险 heredoc 因 bash 未覆盖仍弹，未误放行）**；node typecheck + build 三入口零错误。
- 蓝图勘误（已回写 DESIGN §6.14.4）：原"更深的命令替换嵌套误切属已知边界→多弹卡无害"注记，heredoc 已从该边界清单剔除（现为单块切分）。
- 约束遵守：用户运行实例（pid 521360，18456，98308da）全程未触碰；**修复在主进程 mirrorFilter，需用户重启实例才生效**。
- 收尾三件套：① commit 43cb535 ✅ ② 无新起实例/无孤儿；/tmp 临时测试脚本全清 ✅ ③ 本日志 ✅
- 遗留：无（用户实例重启载新构建后，heredoc 类调查命令不再误弹卡）
