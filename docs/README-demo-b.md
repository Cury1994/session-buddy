...
2. **命令审批打断节奏**。agent 每跑一个命令，终端就弹一次权限确认。人不在旁边的时候，十秒超时，会话干等你；闭眼回车吧，`sudo rm -rf` 那种命令又让人不太放心。

3. **token余量查询麻烦。用 Claude Code 接第三方 API，余额散在几家云平台的网页后台，每张卡一个站点、一套登录，查一次一分钟起步。更糟的是没人会在见底前提醒你，`Insufficient Balance` 总在运行到一半才蹦出来，会话当场凉掉。
...
已内置 **DeepSeek**（按量余额）、**阿里云百炼**（订阅套餐）两张卡。余量源是**可插拔**的——在 `usage_sources` 里配一个 JSON 块就能接新的厂商，无需改代码。

---

## 产品路线图

- **已支持**：Claude Code 会话监控 + 集中审批，生产可用
- **规划中**：Codex CLI 的会话监控，已列入后续计划
- **待验证**：macOS 已完成代码适配，但**未经真机实测**——需要一台 Mac 来验证打包与托盘表现

---

## 真机截图

> 截图待补。项目还没有正式 UI 截图，上方首图是目前最接近产品形态的示意。装上之后自己看一眼就知道什么样，后续版本会补上真实的托盘菜单、会话卡片、余额趋势与审批弹卡截图。

- 首图右侧「驾驶舱面板」即产品核心界面的示意：**会话卡**（状态灯 + ctx%）、**审批弹卡**（危险命令高亮）、**余量趋势卡**（30 天折线）
- 首图底部是托盘图标的**四色状态机**，一眼区分当前状态
